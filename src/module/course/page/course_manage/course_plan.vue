<template>
  <div>
    <el-button type="primary" style="margin: -10px 0 15px 0" @click="teachplayFormVisible = true">添加课程计划</el-button>
    <!--
      data 展示的数据
      props 配置选项
      node-key 唯一标识
    ...
    <el-dialog title="添加课程计划" :visible.sync="teachplayFormVisible" >
      <el-form ref="teachplanForm"  :model="teachplanActive" label-width="140px" style="width:600px;" :rules="teachplanRules" >
        <el-form-item label="上级结点" >
          <el-select clearable v-model="teachplanActive.parentid" placeholder="不填表示根结点">
            <!--在课程计划中还没有节点的时候要自动添加该课程的根结点
              也就是只有课程，点击添加节点将课程名称作为根节点添加。
              然后没有二级节点，不选择添加的就是二级节点。
            -->
            <!--默认显示的是二级节点名称，值为二级节点id
              这里的业务是，如果不选择表明要添加的是二级节点。
              需要给一级节点添加二级节点。
              如果选择添加的是三级节点，给二级节点添加三级节点。
            -->
            <el-option
  ...
  export default {
    ...
    data() {
      return {
        ...
        }],  /*定义对应属性, 对象中的pname会显示，children会作为子树。*/
        defaultProps:{
          /*
          * 指定子树为节点对象的某个属性值,
          * 就是子树是上级节点对象的哪个属性值。
